...
for i in range(opts.iterations):
    ...
    with torch.no_grad():
        output_filename = os.path.join(opts.save_path, f'iter_{i:04d}.obj')
        part_mesh.export(output_filename)
        print(f'Saved current reconstruction to: {output_filename}')
...
with torch.no_grad():
    final_filename = os.path.join(opts.save_path, 'last_recon.obj')
    mesh.export(final_filename)
    print(f'Saved final reconstruction to: {final_filename}')